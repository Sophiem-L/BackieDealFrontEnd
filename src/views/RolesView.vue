  ...
  <v-container fluid class="pa-2">
    ...
    <div class="d-flex align-center justify-space-between" style="padding:0px 14px 14px 14px">
      ...
      <v-btn
        ...
        class="ml-2"
        rounded="lg"
        @click="router.push('/security/roles/create')"
      >
        Create
      </v-btn>
      </div>
          ...
          <v-menu location="bottom end">
            ...
            <v-list density="compact" min-width="160">
              <v-list-item
                ...
                title="Edit"
                base-color="primary"
                @click="router.push(`/security/roles/${item.id}`)"
              />
              <v-list-item
      ...
      <v-card rounded="lg" class="d-flex flex-column" style="height: 80vh;">
        ...
        <v-card-text class="flex-grow-1 overflow-y-auto">
          ...
          <v-card-actions class="pa-4">
            ...
            <v-btn v-if="auth.hasPermission('roles.update')" color="primary" rounded="lg" variant="flat" @click="updatePermission(role.id)"><v-icon left>mdi-content-save</v-icon>Save Permissions</v-btn>
          </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
...
const fetchData = async (opts: DataTableOptions) => {
  ...
  try {
    const params = new URLSearchParams({
      ...
      per_page: String(opts.itemsPerPage),
    });

    const res: any = await apiFetch(`/admin/roles?${params.toString()}`, {
      method: 'GET',
...
const deleteData = async (id: number) => {
    ...
    if (confirmed) {
      try {
        ...
        window.alert(error.message || 'Unable to delete this role.');
      }
    }
};
...
const getRole = async (id: number) => {
    try {
        const res: any = await apiFetch(`/admin/roles/${id}`, {
          token: auth.accessToken
        });
        const data = res.data;
        role.value = {
...
const getPermissions = async () => {
  loading.value = true;
  try {
    const res: any = await apiFetch('/admin/permissions?per_page=100', {
      token: auth.accessToken
    });
    const allPermissions = res.data.items || [];

    const roleRes: any = await apiFetch(`/admin/roles/${selectedRoleId.value}`, {
      token: auth.accessToken
    });
...
const updatePermission = async (id: number | null) => {
    ...
    try {
        ...
        window.alert(error.message || 'Unable to update role permissions.');
    }
    finally {
        loading.value = false;
    }

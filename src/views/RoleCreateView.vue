<template>
  <div class="page">
    <AppHeader title="Create Custom Role" />

    <div class="page__body">
      <p v-if="errors.general" class="alert">{{ errors.general }}</p>

      <!-- Main Form Card (Matches Design Template) -->
      <section class="card">
        ...
        <form @submit.prevent="handleSave" class="card__body">
          <!-- Role Name -->
          ...
          <div class="form-group mt-2">
            ...
            <div v-if="loadingPermissions" class="perm-state">Loading permissions list...</div>
      ...
      <!-- Bottom Floating Action Bar (Back & Save Buttons) -->
      <footer class="bottom-actions">
        ...
        <button type="button" class="btn-save" :disabled="loading" @click="handleSave">
          <svg viewBox="0 0 24 24" fill="none"><path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z" stroke="currentColor" stroke-width="2"/><polyline points="17 21 17 13 7 13 7 21" stroke="currentColor" stroke-width="2"/><polyline points="7 3 7 8 15 8" stroke="currentColor" stroke-width="2"/></svg>
          {{ loading ? 'Saving...' : 'Save' }}
        </button>
      </footer>
    </div>
...
// Fetch all available permissions for initial assignment
async function loadPermissions() {
  loadingPermissions.value = true
  try {
    const res = await apiFetch('/admin/permissions?per_page=150', {
    ...
    allPerms.forEach((p) => {
      ...
      grouped[mod].push({ id: p.id, name: p.name, checked: false })
    })
...
async function handleSave() {
  ...
  try {
    ...
    // Send request matching StoreRoleRequest (name, guard_name, permissions)
...
/* Card UI Matching Template Image */
.card {
  position: relative;
  background: #ffffff;
  ...
  &__accent-bar {
    height: 4px;
    background: #4f46e5; // Indigo accent bar top
  }

...
const statusLabel = computed(() => statusLabels[article.value?.status] ?? article.value?.status)
...
<template>
  <div class="page">
    ...
    <div class="page__body">
      ...
      <div v-else-if="article" class="grid">
        <!-- Main column -->
        <div class="col col--main">
          <section class="card">
            ...
            <div class="field">
              ...
              <p class="field__value field__value--title">{{ article.title }}</p>
            </div>

            <div class="field">
              <span class="field__label">Body</span>
              <p class="field__value field__value--body">{{ article.body || 'No content yet.' }}</p>
            </div>
          </section>
        </div>
        ...
        <div class="col col--side">
          <section class="card">
            ...
            <div class="field">
              ...
              <p class="field__value">
                <span class="badge" :class="`badge--${article.status}`">{{ statusLabel }}</span>
              </p>
            </div>

            <div class="field">
              <span class="field__label">Publish date</span>
              <p class="field__value">{{ formatDate(article.published_at) }}</p>
            </div>

            <div class="field">
              <span class="field__label">Created</span>
              <p class="field__value">{{ formatDate(article.created_at) }}</p>
            </div>

            <div class="field">
              <span class="field__label">Author</span>
              <p class="field__value">{{ article.author?.name ?? 'Unknown' }}</p>
            </div>
          </section>

          <section class="card">
            <h3 class="card__title">Category</h3>
            <p class="field__value">{{ article.category || 'Uncategorised' }}</p>
          </section>
          ...
          <section class="card">
            ...
            ></div>
            <p v-else class="field__value">No cover image.</p>
          </section>
        </div>
      </div>
    </div>
